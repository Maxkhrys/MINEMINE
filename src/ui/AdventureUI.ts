...
export class AdventureUI {
  ...
  private inv!:Inventory; private timer=0; private revision=-1; private fuelSlot=0; private viewKey=''; private lastFocus:Element|null=null;
  ...
  update(dt:number):void{if(!this.open)return;this.timer-=dt;if(this.timer<=0){this.timer=.15;this.refreshMeters();if(this.revision!==this.adventure.revision){if(this.stateKey()===this.viewKey){this.revision=this.adventure.revision;return;}const active=document.activeElement as HTMLElement;const key=active?.dataset.focus;this.render();if(key)this.screen.querySelector<HTMLElement>(`[data-focus="${key}"]`)?.focus();}}}
  private stateKey():string{const d=this.adventure.data;return JSON.stringify([this.kind,this.inv.slots,this.container?.slots,d.backpack,d.equipment,d.stats,d.discovered,d.claimed,d.rescue,d.keepInventory,d.grave,this.adventure.day,this.adventure.phase]);}
  ...
  render():void{
    this.revision=this.adventure.revision;this.viewKey=this.stateKey();
