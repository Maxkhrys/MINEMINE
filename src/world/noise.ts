/**
 * Deterministic random numbers and simplex noise, seeded from the world seed.
 * Used identically by the generation worker and the main thread.
 */
...
/** Turns any seed text into a 32-bit integer. Numeric text maps to its own value. */
...
/** Integer hash of 2D coordinates + seed, returns [0, 1). */
...
export class SimplexNoise {
  ...
  fbm2(x: number, z: number, octaves: number, lacunarity = 2, gain = 0.5): number {
    let sum = 0;
    let amp = 1;
    let freq = 1;
    let norm = 0;
  ...
  /** Ridged multifractal, returns [0, 1] with sharp crests. */
  ridged2(x: number, z: number, octaves: number): number {
    let sum = 0;
    let amp = 0.5;
    let freq = 1;
    let prev = 1;
    let norm = 0;
    for (let o = 0; o < octaves; o++) {
      let n = 1 - Math.abs(this.noise2(x * freq, z * freq));
      n *= n;
      sum += n * amp * prev;
      norm += amp;
      prev = n;
